...
def test_a_clean_acknowledgement_verifies_ten_out_of_ten_on_the_second_channel(
    ...
    assert len(blocks) == 1
    title, checks = blocks[0]
    assert title == f"Verification of {INC} attempt 1 (a.okafor) on the second channel"
    assert len(checks) == 10
    assert all_ok(checks)
    call_id = result.attempts[0].call_id
    assert checks[0][1] == f"second channel returned a run for call {call_id}"
    assert checks[2][1] == f"run echoes attempt id {INC}/primary/1"
    assert checks[4][1] == "run status COMPLETED maps to the recorded completed"
    assert checks[8][1] == "the recorded ETA of 15 minutes is spoken by the recipient"
...
def test_the_channel_mismatch_scenario_fails_six_of_ten_checks(serving, rest_client, mcp_client):
    ...
    checks = blocks[0][1]
    assert [ok for ok, _ in checks] == [True] * 5 + [False] * 4 + [True]
    assert not all_ok(checks)
    assert "verified 6/10" in render_blocks(blocks)


def test_a_run_the_second_channel_cannot_see_yields_a_single_failed_check(
    serving, rest_client, mcp_client
):
    server, result = settle(
    ...
    checks = blocks[0][1]
    assert checks == [(False, f"second channel returned a run for call {result.attempts[0].call_id}")]
    assert not all_ok(checks)
...
def test_a_run_that_finished_outside_the_escalation_window_fails_closed(
    ...
    checks = verify_ladder(mcp_client(server), INC, result, past)[0][1]

    assert checks[9] == (False, "the run finished inside the escalation window")
...
def test_an_unparseable_completion_time_fails_closed(serving, rest_client, mcp_client):
    ...
    checks = verify_ladder(mcp_client(server), INC, result, wide_window())[0][1]
